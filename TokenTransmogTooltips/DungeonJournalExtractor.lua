...
function DungeonJournalExtractor:ExtractTokenData()
	...
	for i = 1, EJ_GetNumLoot() do
		...
		if
			itemInfo
			and itemInfo.filterType == Enum.ItemSlotFilterType.Other
			and itemInfo.itemQuality == epicHex
			and itemInfo.displaySeasonID == nil
		then
...
function DungeonJournalExtractor:Initialize()
	...
	local function UpdateButtonVisibility()
		if
			EncounterJournal:IsShown()
			and EncounterJournal.encounter
			and EncounterJournal.encounter.info
			and EncounterJournal.encounter.info.tab
			and EncounterJournal.encounter.info.lootTab
			and EncounterJournal.encounter.info.tab == EncounterJournal.encounter.info.lootTab:GetID()
		then
			extractButton:Show()
		else
			extractButton:Hide()
