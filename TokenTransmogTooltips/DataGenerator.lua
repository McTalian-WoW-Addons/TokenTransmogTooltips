	...
	if frame.currentMode == MODE_ITEM_LIST then
		...
		if #items == 0 then
			frame.outputEditBox:SetText(
				'Error: Could not parse any items from the input.\n\nExpected format (one per line):\n  itemId, ARMOR_TYPE, SLOT\n  { itemId, "ARMOR_TYPE", "SLOT" },'
			)
			return
		end
		...
		if setInfo.label == label then
			...
			for _, slotInfo in ipairs(selectedSlots) do
				...
				if sources and #sources > 0 then
					...
					table.insert(
						templateOutput,
						string.format("%s, %d, %d", slotInfo.name, source.visualID, source.sourceID)
					)

					if #sources > 1 then
						table.insert(
							auditOutput,
							string.format("### %s - %s - %s - %s", classFile, setName, setDescription, slotInfo.name)
						)
...
function DataGenerator:GenerateItemListData(items)
	...
	for _, entry in ipairs(items) do
		...
		if appearanceID then
			if not map[entry.armorType] then
				map[entry.armorType] = {}
			end
			if not map[entry.armorType][entry.slot] then
				map[entry.armorType][entry.slot] = {}
			end
...
function DataGenerator:SetMode(mode)
	...
	if mode == MODE_SET_LABEL then
		local lastSlotLeftSide = #frame.slotCheckboxes % 2 == 0 and frame.slotCheckboxes[#frame.slotCheckboxes - 1]
